...
void DeviceManager::writeSportModes(const QList<QVariantMap> &modes)
{
    mutex.lock();
    if (deviceObject == NULL) {
        mutex.unlock();
        emit sportModeWriteResult(false);
        return;
    }
    ...
    if (libambit_malloc_sport_modes(modes.count(), settings)) {
        ambit_sport_mode_t *slot = settings->sport_modes;
        for (QVariantMap m : modes) {
            CustomMode cm(m);
            cm.toAmbitCustomModeData(slot, settings);
            slot++;
        }
    }

    int res = libambit_sport_mode_write(deviceObject, settings);
    libambit_sport_mode_device_settings_free(settings);
    mutex.unlock();

    emit sportModeWriteResult(res == 0);
}
