    ...
    currentLogMessageRow(NULL)
{
    ...
    ui->labelFirmwareVersion->setHidden(true);
    ...
    // Setup map — centre on saved view or on Plumergat (home) if no GeoJSON yet
    ui->mapView->loadMap();
    ui->mapView->applyInitialView(GeoJSONExporter::hasAnyFiles());
    connect(ui->buttonFitAll, SIGNAL(clicked()), ui->mapView, SLOT(fitAll()));
    connect(ui->layerTree, &QTreeWidget::itemChanged,
            this, &MainWindow::onLayerTreeItemChanged);

    // Setup sport mode editor
    ui->sportModeEditor->setModes(SportModeStorage::load());
    connect(ui->sportModeEditor, &SportModeEditorWidget::writeToWatchRequested,
            this, &MainWindow::onWriteSportModesToWatch);
    connect(deviceManager, &DeviceManager::sportModeWriteResult,
            this, [this](bool ok) {
        QMessageBox::information(this, tr("Sport Modes"),
            ok ? tr("Sport modes written to watch successfully.")
               : tr("Failed to write sport modes — is the watch connected?"));
    });

    // Setup Movescount
    movesCountSetup();

    // Refresh GPS orbit cache on startup if stale (deferred so the window appears first)
    QTimer::singleShot(2000, this, SLOT(autoRefreshOrbitIfStale()));
}
...
void MainWindow::showWorkoutBuilder()
{
    WorkoutEditorDialog *dialog = new WorkoutEditorDialog(this);
...
void MainWindow::onWriteSportModesToWatch(const QList<QVariantMap> &modes)
{
    ...
    QMetaObject::invokeMethod(deviceManager, "writeSportModes",
                              Qt::QueuedConnection,
                              Q_ARG(QList<QVariantMap>, modes));
}
...
void MainWindow::deviceDetected(const DeviceInfo& deviceInfo)
{
    ...
    ui->labelFirmwareVersion->setText(QString("FW: %1.%2.%3")
        .arg(deviceInfo.fw_version[0])
        .arg(deviceInfo.fw_version[1])
        .arg(deviceInfo.fw_version[2]));
    ui->labelFirmwareVersion->setHidden(false);
...
void MainWindow::autoRefreshOrbitIfStale()
{
    // Only auto-refresh for the Open IGS source (index 0)
    settings.beginGroup("orbitSettings");
    int src = settings.value("orbitSource", 0).toInt();
    settings.endGroup();
    if (src != 0)
        return;
    ...
    QFileInfo fi(cache);
    if (fi.exists()) {
        qint64 ageSeconds = fi.lastModified().secsTo(QDateTime::currentDateTime());
        if (ageSeconds < 86400)   // less than 1 day old → nothing to do
            return;
    }
    ...
    ui->labelOrbitAge->setText(tr("GPS orbit cache: refreshing in background…"));
    ...
    connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
        ...
        if (code == 0) {
            ...
            ui->labelOrbitAge->setText(tr("GPS orbit cache: auto-refresh failed"));
        }
        proc->deleteLater();
    });
...
void MainWindow::updateOrbitAgeLabel()
{
    QString cache = QString(getenv("HOME")) + "/.openambit/orbit_cache.uo";
    QFileInfo fi(cache);
    if (fi.exists()) {
        QDateTime mod = fi.lastModified();
        qint64 ageMinutes = mod.secsTo(QDateTime::currentDateTime()) / 60;
        QString ageStr;
        if (ageMinutes < 60)
            ageStr = QString("%1 min ago").arg(ageMinutes);
        else if (ageMinutes < 1440)
            ageStr = QString("%1 h ago").arg(ageMinutes / 60);
        else
            ageStr = QString("%1 days ago").arg(ageMinutes / 1440);
        ui->labelOrbitAge->setText(QString("GPS orbit cache: %1 (%2 KB, %3)")
            .arg(mod.toString("yyyy-MM-dd hh:mm"))
            .arg(fi.size() / 1024)
            .arg(ageStr));
    } else {
        ui->labelOrbitAge->setText(tr("GPS orbit cache: not downloaded yet"));
    }
}
...
void MainWindow::logItemSelected(QListWidgetItem *current,QListWidgetItem *previous)
{
    ...
    if (current != NULL) {
        QString logId = current->data(Qt::UserRole).toString();
        logEntry = logStore.read(logId);
        if (logEntry != NULL) {
            ui->logDetail->showLog(logEntry);
            QString trackGeoJSON = GeoJSONExporter::exportTrack(logEntry);
            if (!trackGeoJSON.isEmpty()) {
                refreshLayerTree(logId, trackGeoJSON);
            }
        }
        delete logEntry;
    }
...
void MainWindow::refreshLayerTree(const QString &logId, const QString &trackGeoJSON)
{
    static QMap<QString, QString> loadedLayers;

    QString trackId = logId + "_track";
    loadedLayers[trackId] = trackGeoJSON;

    ui->mapView->loadGeoJSON(trackId, trackGeoJSON, "#3584e4");

    // Find or create the Tracks parent
    QTreeWidgetItem *tracksParent = nullptr;
    for (int i = 0; i < ui->layerTree->topLevelItemCount(); i++) {
        if (ui->layerTree->topLevelItem(i)->text(0) == "Tracks") {
            tracksParent = ui->layerTree->topLevelItem(i);
            break;
        }
    }
    if (!tracksParent) {
        tracksParent = new QTreeWidgetItem(ui->layerTree, QStringList("Tracks"));
        tracksParent->setFlags(tracksParent->flags() | Qt::ItemIsUserCheckable);
        tracksParent->setCheckState(0, Qt::Checked);
    }

    // Check if child already exists for this log
    QTreeWidgetItem *child = nullptr;
    for (int i = 0; i < tracksParent->childCount(); i++) {
        if (tracksParent->child(i)->data(0, Qt::UserRole).toString() == trackId) {
            child = tracksParent->child(i);
            break;
        }
    }
    if (!child) {
        // Parse the name from GeoJSON
        QJsonDocument doc = QJsonDocument::fromJson(trackGeoJSON.toUtf8());
        QString name = logId;
        if (!doc.isNull()) {
            auto feats = doc.object()["features"].toArray();
            if (!feats.isEmpty())
                name = feats[0].toObject()["properties"].toObject()["name"].toString(logId);
        }
        child = new QTreeWidgetItem(tracksParent, QStringList(name));
        child->setData(0, Qt::UserRole, trackId);
        child->setFlags(child->flags() | Qt::ItemIsUserCheckable);
        child->setCheckState(0, Qt::Checked);
    }
    tracksParent->setExpanded(true);

    ui->mapView->fitAll();
}

void MainWindow::onLayerTreeItemChanged(QTreeWidgetItem *item, int column)
{
    if (column != 0) return;
    QString layerId = item->data(0, Qt::UserRole).toString();
    if (layerId.isEmpty()) return;
    ui->mapView->showLayer(layerId, item->checkState(0) == Qt::Checked);
}
...
void MainWindow::movesCountSetup()
{
    ...
    settings.beginGroup("orbitSettings");
    if (movesCount != NULL) {
        movesCount->setOrbitSource(settings.value("orbitSource", 0).toInt());
        movesCount->setOrbitLocalPath(settings.value("orbitLocalPath", "").toString());
    }
    settings.endGroup();
}
