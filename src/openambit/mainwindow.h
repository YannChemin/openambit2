...
private slots:
    ...
    void updateLogList();
    void onLayerTreeItemChanged(QTreeWidgetItem *item, int column);
    void onWriteSportModesToWatch(const QList<QVariantMap> &modes);

private:
    void startSync();
    void movesCountSetup();
    void updateOrbitAgeLabel();
    void refreshLayerTree(const QString &logId, const QString &trackGeoJSON);
