...
#define GPS_ORBIT_DATA_MIN_SIZE 1000 /* byte — lowered to accept broadcast ephemeris (~4KB) */
...
void MovesCount::setOrbitSource(int source)
{
    this->orbitSource = source;
}

void MovesCount::setOrbitLocalPath(QString path)
{
    this->orbitLocalPath = path;
}
...
static QByteArray readOrbitFile(const QString &path)
{
    QFile f(path);
...
static QString findOrbitScript()
{
...
int MovesCount::getOrbitalDataInThread(u_int8_t **data)
{
    ...
    if (orbitSource == 2) {
        // Legacy Movescount
        qDebug() << "Fetching orbital data from Movescount";
        QNetworkReply *reply = syncGET("/devices/gpsorbit/binary", "", false);
        if (reply->error() == QNetworkReply::NoError)
            _data = reply->readAll();
        else
            qDebug() << "Failed to fetch orbital data from Movescount (err:" << reply->error() << ")";
        delete reply;

    } else if (orbitSource == 1) {
        // Local file
        qDebug() << "Reading orbital data from local file:" << orbitLocalPath;
        _data = readOrbitFile(orbitLocalPath);
        if (_data.isEmpty())
            qDebug() << "Failed to read orbit file:" << orbitLocalPath;

    } else {
        // Open IGS via rinex2ubx_ambit.py (default)
        QString cache = QString(getenv("HOME")) + "/.openambit/orbit_cache.uo";
        QString script = findOrbitScript();

        if (script.isEmpty()) {
            qDebug() << "rinex2ubx_ambit.py not found; falling back to cached orbit file";
        } else {
            qDebug() << "Fetching orbital data via Open IGS (script:" << script << ")";
            QProcess proc;
            proc.start("python3", QStringList() << script);
            if (proc.waitForFinished(120000)) {
                if (proc.exitCode() != 0)
                    qDebug() << "rinex2ubx_ambit.py failed:" << proc.readAllStandardError();
                else
                    qDebug() << "rinex2ubx_ambit.py:" << proc.readAllStandardOutput().trimmed();
            } else {
                qDebug() << "rinex2ubx_ambit.py timed out";
                proc.kill();
            }
        }

        _data = readOrbitFile(cache);
        if (_data.isEmpty())
            qDebug() << "No orbit cache file found at" << cache;
    }

    if (_data.length() >= GPS_ORBIT_DATA_MIN_SIZE) {
        *data = (u_int8_t*)malloc(_data.length());
        memcpy(*data, _data.data(), _data.length());
        qDebug() << "Orbit data ready, length:" << _data.length();
        return _data.length();
    }

    qDebug() << "Orbit data too small or missing (length:" << _data.length() << ")";
    return -1;
}
